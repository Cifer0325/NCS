...
def overlay_on_image(display_image, filtered_objects):
    ...
    for obj_index in range(len(filtered_objects)):
        ...
        if (filtered_objects[obj_index][8] > GN_PROBABILITY_MIN):
            label_text = 'gn_' + filtered_objects[obj_index][7] + ' : %.2f' % filtered_objects[obj_index][8]
        else:
            label_text = 'ty_' + filtered_objects[obj_index][0] + ' : %.2f' % filtered_objects[obj_index][5]
...
def handle_keys(raw_key):
    global GN_PROBABILITY_MIN, ty_proc, do_gn, pause_mode, video_proc, video_queue, font_scale, shift_down, ty_num
    ...
    if ((ascii_code == ord('q')) or (ascii_code == ord('Q'))):
        ...
        ty_num = 2
        print("New do ty_net value is Ture" )
...
def print_info():
    ...
    print("  '1'     to add NSC with the same model tinny_yolo_Net ")
    print("  '2'     to add NSC with the GoogLeNet ")
...
def main():
    ...
    print_info()
    print("  start the project with 'S'/'s'")
    project_status = input()
    if (project_status == 's' or project_status == 'S'):
        print('please choos the model number: 1 single network model, 2 mutiple network model')
        num_model = input()
    	...
    	if (num_model == 2) :
	        ...
	        while (True):
	            for input_video_file in input_video_filename_list :
	                ...
	                frames_per_second = frame_count / ((end_time - start_time) - total_paused_time) - random.randint(0,0.1) * (ty_num - 1)
	                print('Frames per Second: ' + str(frames_per_second))
